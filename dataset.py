...
def temporal_align_aux_sensors(main_sensor_timestamps, aux_sensor_timestamp, max_diff=2):
    differences = [abs(dt - aux_sensor_timestamp)
                   for dt in main_sensor_timestamps]
...
class AllClearDataset(Dataset):
    ...
    def __init__(self,
        ...
        if selected_rois == "all":
            ...
            self.dataset = {ID: info for ID, info in dataset.items(
            ) if info["roi"][0] in selected_rois}
    ...
    def preprocess(self, image, sensor_name):
        ...
        if sensor_name in ["s2_toa", "landsat8", "landsat9"]:
            ...
            print(
                f'Preprocessing steps for {sensor_name} has not been implemented yet.')
            image = image
        return image
    ...
    def __getitem__(self, idx):
        ...
        for sensor in self.sensors:
            ...
            for sensor_input in sensor_inputs:
                ...
                image = self.load_and_center_crop(
                    fpath, self.channels[sensor], self.center_crop_size)
                ...
                if sensor == self.main_sensor:
                    ...
                    if "cld_shdw" in self.aux_data:
                        ...
                        if os.path.exists(cld_shdw_fpath):
                            try:
                                ...
                                raise ValueError(f"Invalid contents in {cld_shdw_fpath}")
                        else:
                            raise FileNotFoundError(cld_shdw_fpath)
                    ...
                    if "dw" in self.aux_data:
                        ...
                        if os.path.exists(dw_fpath):
                            dw = self.load_and_center_crop(
                                dw_fpath, self.channels["dw"], self.center_crop_size)
        ...
        timestamps_main_sensor = [
            timestamp for timestamp, _ in inputs[self.main_sensor]]
        ...
        if self.format == "stp":  # TODO: refactor this code once more format is added.
            inputs_main_sensor = torch.stack(
                [image for _, image in inputs[self.main_sensor]])
            if "cld_shdw" in self.aux_data:
                inputs_cld_shdw = torch.stack(
                    [cld_shdw for _, cld_shdw in inputs["input_cld_shdw"]])
            else:
                inputs_cld_shdw = None
            if "dw" in self.aux_data:
        ...
        if self.target_mode == "s2p":
            ...
            if not os.path.exists(
                    fpath):  # Add this line to handle the case when the script is not running on Sun / Bala's server
                fpath = fpath.replace(
                    "/scratch/allclear/dataset_v3/", "/share/hariharan/cloud_removal/MultiSensor/")
            image = self.load_and_center_crop(
                fpath, self.channels[self.main_sensor], self.center_crop_size)
            # target by default is the main sensor
            ...
            if "cld_shdw" in self.aux_data:
                ...
                if os.path.exists(cld_shdw_fpath):
                    ...
                    raise ValueError(
                        f"Cloud shadow file not found: {cld_shdw_fpath}")
            ...
            if "dw" in self.aux_data and fpath.endswith("tif"):
                ...
                if not os.path.exists(dw_fpath):
                    dw_fpath = fpath.rsplit(
                        '/', 1)[0].rsplit('/', 1)[0].rsplit('/', 1)[0] + "/2022_*/dw/*.tif"
                    dw_fpaths = glob.glob(dw_fpath)
                    given_date = datetime.strptime(
                        timestamp, "%Y-%m-%d %H:%M:%S") if isinstance(timestamp, str) else timestamp
                    dw_fpath = min(dw_fpaths, key=lambda path: abs(
                        self.extract_date(path) - given_date))
        ...
        if self.format != "stp":
            ...
            # avoid adding landsat8 and landsat9 separately
            if "landsat8" in self.sensors and "landsat9" in self.sensors:
                stp_sensors = [
                    sensor for sensor in self.sensors if sensor != "landsat8"]
            else:
                stp_sensors = output_sensors
            sample_stp = []

            for sensor in stp_sensors:
                sample_stp.append(torch.ones(
                    (self.tx, len(self.channels[sensor]), *self.center_crop_size)))
            ...
            if self.target_mode == "s2p":
                ...
                target_timestamps = datetime.strptime(
                    inputs["target"][0][0], "%Y-%m-%d %H:%M:%S").timestamp()
            ...
            item_dict = {
                ...
                # Shape: (1, T, H, W)
                # Shape: (C, T, H, W)
                "input_dw": inputs_dw.permute(1, 0, 2, 3) if inputs_dw is not None else None,
